...
function txt() {
  var t   =  "OVERPOPULATED".split("").join(String.fromCharCode(0x2004));
  ctx.font = "4em winter";
  ctx.fillStyle = '#7B6C69';
...
function anim() {
  ...
  sin = Math.sin(ang);
  cos = Math.cos(ang);
  var g = ctx.createRadialGradient(c.width, c.width,0, c.height, c.height, c.width);
  g.addColorStop(0,'hsla(40, 6.7%, 91.2%,1)');
  g.addColorStop(0.5, 'hsla(0, 5.3%, 96.3%,1)');
  g.addColorStop(1, 'hsla(48, 7%, 86.1%,1)');
  ctx.fillStyle = g;
  ...
  while (p != null) {
    ...
    if (rng || p.end)fin(p);
      ...
      ctx.fillStyle = 'hsla(265.7, 3.9%, 64.9%, '+p.alpha+')';
      ctx.beginPath();
